function [SP, SPraw] = parse_to_pumpheights_ARM_2pump_V2(SPraw, DeploymentInfo, daterange, rmvfirst2npoints)
...
% Edits by Ben Werb at MBARI.
% Modified: 3/5/2025 to read in data using new data parser GetMFETData
...
nreps = nanmax(SPraw.Number_of_Samples_per_depth);
npumps = nanmax(SPraw.pumpnum);
sampinterval = DeploymentInfo.sampinterval;
min2avg = sampinterval * nreps / 60;
...
% vars = vertcat(vars, {'DOXY'; 'pHext'}); % removed pHint, PAR
...
istart = find(SPraw.pumpnum == 1 & SPraw.CycleNumber == 1, 1, 'first');
iend = find(SPraw.pumpnum == npumps & SPraw.CycleNumber == nreps, 1, 'last');
SPraw = SPraw(istart:iend,:);
...
vars = {'SDN', 'pHext', 'TC', 'DOXY', 'PSAL', 'Pres', 'O2SATPER', 'OPT_TC', 'MCAT_TC'}; % removed pHint, DFET_TC, PAR
SPraw_table = SPraw;

SPraw = table2struct(SPraw,"ToScalar",true);

SPraw.TC = SPraw.OPT_TC; % why do this?
...
for i = 1:length(SPraw.pumpnum) 
    if(SPraw.pumpnum(i) == 1 && SPraw.CycleNumber(i) == 1)
        ip1start = [ip1start; i];
    elseif(SPraw.pumpnum(i) == 1 && SPraw.CycleNumber(i) == nreps)
        ip1end = [ip1end; i];
    elseif(SPraw.pumpnum(i) == 2 && SPraw.CycleNumber(i) == 1)
        ip2start = [ip2start; i];
    elseif(SPraw.pumpnum(i) == 2 && SPraw.CycleNumber(i) == nreps)
        ip2end = [ip2end; i];
...
% changing rmoutliers to rmoutliers
...
%         vec3in = SPraw.(vars{v})(ip3start(j)+2:ip3end(j)); 
        vec1out = rmoutliers(vec1in); 
        vec2out = rmoutliers(vec2in);
%         vec3out = rmoutliers(vec3in);
